...
void printHelp(void)
{
    printf("usage: flash2dfu <inputBinFlash.bin>\r\n");
...
int main(int argc,char ** argv)
{
    ...
    if (outputFileName == 0)
    {
        outputFileName = "PPFX_FirstTime.dfu";
    }
    inputFileBinFlash = fopen(inputFileNameBinFlash,"rb");
    outputFile = fopen(outputFileName,"wb");

    fwrite("DfuSe",5,1,outputFile); //szSignature
    bytesBfr[0]=1;
    fwrite(bytesBfr,1,1,outputFile); //bVersion


    fseek(inputFileBinFlash,0,SEEK_END);
    wordBfr.wordVal = (uint32_t)ftell(inputFileBinFlash);
    fseek(inputFileBinFlash,0,SEEK_SET);
    wordBfr.wordVal += (16+11+274+8);
    fwrite(wordBfr.byteVal+0,1,1,outputFile);
    fwrite(wordBfr.byteVal+1,1,1,outputFile);
    fwrite(wordBfr.byteVal+2,1,1,outputFile);
    fwrite(wordBfr.byteVal+3,1,1,outputFile); // DFUImageSize 
    fwrite(bytesBfr,1,1,outputFile); // again 1 as bTargets
    wordBfr.wordVal -= (16+11+274+8);

    // write target prefix of first and only image
    fwrite("Target",6,1,outputFile); //szSignature
    bytesBfr[0]=0;
    bytesBfr[1]=1;
    bytesBfr[2]=0;
    bytesBfr[3]=0;
    bytesBfr[4]=0;
    fwrite(bytesBfr,5,1,outputFile);// bAlternateSetting, 0 is internal flash
                                    // bTargetNamed is true
    fwrite("PPFX Internal Flash for first-time/rescue flashing",51,1,outputFile); // szTargetName: zeros since target is not named
    for (uint16_t c=0;c<256;c++)
    {
        *(bytesBfr+c)=0;
    }
    fwrite(bytesBfr,255-51,1,outputFile);
    wordBfr.wordVal += 8; // file size plus image address and image element size
    fwrite(wordBfr.byteVal+0,1,1,outputFile);
    fwrite(wordBfr.byteVal+1,1,1,outputFile);
    fwrite(wordBfr.byteVal+2,1,1,outputFile);
    fwrite(wordBfr.byteVal+3,1,1,outputFile); // dwTargetSize
    bytesBfr[0]=1;
    bytesBfr[1]=0;
    bytesBfr[2]=0;
    bytesBfr[3]=0;
    fwrite(bytesBfr,4,1,outputFile); // dwNbElements
    bytesBfr[0]=0x0;
    bytesBfr[1]=0x0;
    bytesBfr[2]=0x0;
    bytesBfr[3]=0x08;
    fwrite(bytesBfr,4,1,outputFile); //dwElementAddress
    wordBfr.wordVal -= 8;
    fwrite(wordBfr.byteVal+0,1,1,outputFile);
    fwrite(wordBfr.byteVal+1,1,1,outputFile);
    fwrite(wordBfr.byteVal+2,1,1,outputFile);
    fwrite(wordBfr.byteVal+3,1,1,outputFile); // dwElementSize 



    for (uint32_t c=0;c<((wordBfr.wordVal/CHUNK_SIZE)+1);c++)
    {
        uint32_t bytesRemaining = wordBfr.wordVal - c*CHUNK_SIZE;
    ...
    bytesBfr[10] = __HIBYTE(USB_VENDOR_ID_BOOTMODE);
    bytesBfr[11] = __LOBYTE(USB_VENDOR_ID_BOOTMODE);
    bytesBfr[12] = __HIBYTE(USB_PRODUCT_ID_BOOTMODE);
    bytesBfr[13] = __LOBYTE(USB_PRODUCT_ID_BOOTMODE);
    bytesBfr[14] = 0x1;
    bytesBfr[15] = 0x0;
    ...
    wordBfr.wordVal = (uint32_t)ftell(outputFile);
    rewind(outputFile);
    crc = 0xffffffff;  
    for (; wordBfr.wordVal; wordBfr.wordVal--)
