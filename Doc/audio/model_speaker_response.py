...
def highpassed_ir(ir, f_cutoff, f_sample):
    ...
    for i in range(ir.size):
        if (0 <= f_scale[i] < f_cutoff) or (0 > f_scale[i] > -f_cutoff):
            spectr[i] = max_spectr
...
def plot_model_cab_curve(sampling_rate=44100, axes=None, style="-k", sample_length=4096):
    ...
    [b, a] = scipy.signal.butter(iir_lowpass_order, iir_lowpass_cutoff,
                                 btype="low", analog=False, output="ba", fs=sampling_rate)
    [b_out, a_out] = scipy.signal.butter(iir_lowpass_order, iir_lowpass_cutoff,
                                         btype="low", analog=False, output="ba", fs=48000)
    b_out = b_out*32767.
    a_out = a_out*32767.
    print("butterworth lowpass @{}Hz,\r\n\tb: {}\r\n\ta: {}".format(
        iir_lowpass_cutoff, b_out.astype("int16"), a_out.astype("int16")))
    ...
    [b, a] = scipy.signal.cheby2(iir_highpass_order, iir_highpass_atten, iir_highpass_cutoff,
                                 btype="high", analog=False, output="ba", fs=sampling_rate)
    [b_out, a_out] = scipy.signal.cheby2(iir_highpass_order, iir_highpass_atten, iir_highpass_cutoff,
                                         btype="high", analog=False, output="ba", fs=48000)
    b_out = b_out*32767.
    a_out = a_out*32767.
    print("chebychev highpass @{}Hz with attenuation {}dB\r\n\tb: {}\r\n\ta: {}".format(
        iir_highpass_cutoff, iir_highpass_atten, b_out.astype("int16"), a_out.astype("int16")))
    ...
    low_peak_order = 2
    low_peak_mix = 0.0
    [b, a] = scipy.signal.cheby2(low_peak_order, low_peak_atten,
                                 [low_peak_freq-low_peak_width, low_peak_freq+low_peak_width],
                                 btype="bandpass", analog=False, output="ba", fs=sampling_rate)
    wzb, hzn = scipy.signal.freqz(b, a, fs=sampling_rate, worN=sample_length)
    hzn = hzn / max(abs(hzn))
...
def plot_simple_model_cab_curve(axes=None, style="-k"):
    ...
    midcut_atten = 10
    midcut_fact = 0.98
    [b, a] = scipy.signal.cheby1(midcut_order, midcut_atten, midcut_freqs,
                                 btype="bandstop", analog=False, output="ba", fs=48000)
    #b=-b
