...
def design_iir_filter(rs=20, fc=None, fs=48000, do_overflow=True, sample_size=16, scaling=None, filtertype="cheby2",
    ...
    if type == "cheby2":
        second_order_section = signal.cheby2(order, rs, fc, analog=False, btype=filtertype, output='sos', fs=fs)
        ba = signal.butter(order, fc, analog=False, btype=filtertype, output='ba', fs=fs)
    elif type == "cheby1":
        second_order_section = signal.cheby1(order, rs, fc, analog=False, btype=filtertype, output='sos', fs=fs)
        ba = signal.butter(order, fc, analog=False, btype=filtertype, output='ba', fs=fs)
    elif type == "butter":
        second_order_section = signal.butter(order, fc, analog=False, btype=filtertype, output='sos', fs=fs)
        ba = signal.butter(order, fc, analog=False, btype=filtertype, output='ba', fs=fs)
    else:
...
def design_and_plot_iir_filter(do_plot=True,rs=20,fc=None,fs=48000,do_overflow=True,sample_size=16,scaling=None,type="cheby2",ftype="lowpass"):
    ...
    plot_iir_filter(sos, do_plot=do_plot, fs=fs, do_overflow=do_overflow, sample_size=sample_size, scaling=scaling, fc=fc, filtertype=ftype, passtype=type, rs=rs)
