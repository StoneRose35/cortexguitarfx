...
void sendOverUsb(uint8_t * data,uint16_t dlen,uint8_t blocking)
{
    ...
    chunksSent = 0;
    sendDataSize =dlen;
    if ((bmUsbStatus & (1 << USB_CDC_Configured_Pos)) != 0 )
    {
...
void UsbCdcDataReceived(void* dataPtr,uint16_t len)
{
    ...
    while (((receivedDataLevel - receivedDataLevelSnapshot) & USB_CDC_RR_BUFFER_SIZE) < USB_CDC_RR_BUFFER_SIZE && c < len)
    {
        *(receivedDataBfr + (receivedDataLevel++)) = *((uint8_t*)dataPtr + c);
        receivedDataLevel &= USB_CDC_RR_BUFFER_SIZE; 
        c++;
    }
    
    if (((receivedDataLevel - receivedDataLevelSnapshot) & USB_CDC_RR_BUFFER_SIZE) < (USB_CDC_RR_BUFFER_SIZE + 1) - USB_CDC_DATA_OUT_PACKETSIZE)
    {
        prepareUSBReception(1,USB_CDC_DATA_OUT_PACKETSIZE);
    }
...
{
    return (receivedDataLevel - receivedDataLevelSnapshot) & USB_CDC_RR_BUFFER_SIZE;
}

__QSPI_CODE
uint16_t readUsbCdcData(uint8_t * data,uint16_t startIndex)
{
    uint16_t c=0;
    while(receivedDataLevelSnapshot != receivedDataLevel)
    {
        *(data + ((c+startIndex) & USB_CDC_RR_BUFFER_SIZE)) = *(receivedDataBfr + receivedDataLevelSnapshot++);
        receivedDataLevelSnapshot &= USB_CDC_RR_BUFFER_SIZE;
        c++;
    }
