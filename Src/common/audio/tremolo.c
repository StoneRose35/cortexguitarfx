...
int16_t tremoloProcessSample(int16_t sample,TremoloType*data)
{
    ...
    currentAmpValue = ((data->currentLfoVal*data->lfoUpdateCnt) >> TREMOLO_LFO_UPDATE_DIV_POW2)
        + ((data->nextLfoVal*(TREMOLO_LFO_UPDATE_DIV-data->lfoUpdateCnt))>> TREMOLO_LFO_UPDATE_DIV_POW2); 

    currentAmpValue = (1 << 15) + (((currentAmpValue*data->depth) >> 15) - (((1 << 15)*data->depth) >> 15));
    if (data->lfoUpdateCnt == (TREMOLO_LFO_UPDATE_DIV - 1))
    {
        data->currentLfoVal = data->nextLfoVal;
        data->nextLfoVal = phaseDistortedSineSquareNextSample(&data->modulator);
    }
    data->lfoUpdateCnt++;
    data->lfoUpdateCnt &= TREMOLO_LFO_UPDATE_DIV-1;
...
void initTremolo(TremoloType*data)
{
    ...
    data->modulator.phaseIncrement = (1 << 16)*3/AUDIO_SAMPLING_RATE*TREMOLO_LFO_UPDATE_DIV; // 3 Hz for effective sampling rate
