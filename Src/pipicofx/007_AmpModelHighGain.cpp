 ...
 float AmpModelHighGain::AmpModelHighGain::processSample(float sampleIn)
{
    ...
    this->highpass_out =  (1.0f + this->highpassCutoff)/2.0f*(sampleIn - this->highpass_old_in) + this->highpassCutoff*this->highpass_old_out; //(((((1 << 15) + pData->highpassCutoff) >> 1)*(sampleIn - pData->highpass_old_in))>>15) + ((pData->highpassCutoff *pData->highpass_old_out) >> 15);
...
/*
__QSPI_CODE
 void fxProgram9Reset(void*data)
{
