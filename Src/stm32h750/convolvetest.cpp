...
uint32_t firfiltertest()
{
    ...
    for (uint8_t c=0;c<64;c++)
    {
        if (c==0)
        {
            fout = firFilterProcessSample(8388607.0f,&p1->filter3);
        }
        else
        {
            fout = firFilterProcessSample(0.0f,&p1->filter3);
        }
        if ((fout - (8388607.0f*p1->filter3.coefficients[c])) < -0.001f || (fout - (8388607.0f*p1->filter3.coefficients[c])) > 0.001f)
        {
            failures++;
        }
    }
